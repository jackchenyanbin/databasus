    ...
    <div className="mb-5 w-full rounded-tr-md rounded-br-md rounded-bl-md bg-white p-3 shadow sm:p-5 dark:bg-gray-800">
      ...
      <div className="mt-3 flex flex-col gap-2 sm:mt-4 sm:flex-row sm:items-center">
        ...
        <Select
          ...
          options={[
            { value: 'today', label: t('healthcheck.today') },
            { value: '7d', label: t('healthcheck.last7Days') },
            { value: '30d', label: t('healthcheck.last30Days') },
            { value: 'all', label: t('healthcheck.allTime') },
          ]}
        />
      </div>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-wrap gap-1">
          {healthcheckAttempts.length > 0 ? (
            ...
            <div className="text-xs text-gray-400">{t('healthcheck.noDataYet')}</div>
          )}
        </div>
      )}
    </div>
